...
class View_Transform_Layer(tf.keras.Model):
    ...
    def model(self, gait_encode_shape: int, view_onehot_shape: int) -> tf.keras.models:
        gait_encode_input = tf.keras.Input(
            shape=(gait_encode_shape), name='gait_encode_input')
        view_onehot_input = tf.keras.Input(
            shape=(view_onehot_shape), name='view_onehot_input')
