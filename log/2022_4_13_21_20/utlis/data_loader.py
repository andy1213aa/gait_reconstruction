...
class OU_MVLP(GenericTFLoader):
    ...
    def parse(self, example_proto):
        ...
        ang1_idx, ang2_idx = np.random.choice(
            range(self._config['resolution']['angle_nums']), 2)

        angle_onehot = tf.one_hot(
            tf.range(self._config['resolution']['angle_nums']),
            self._config['resolution']['angle_nums'])
